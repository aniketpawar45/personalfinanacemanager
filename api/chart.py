...
def resolve_flexible_bounds(query_str):
    ...
    if query_clean in months_map:
        ...
        # Calculate start and end for that month in the current year (2026)
        year = now.year
        ...
        if target_month == 12:
            ...
            end = datetime.datetime(year, target_month + 1, 1, 0, 0, 0, tzinfo=now.tzinfo) - datetime.timedelta(microseconds=1)
...
async def handle_chart_command(bot, chat_id, command, uid):
    ...
    if not stats or not stats.get('categories'):
        await bot.send_message(chat_id, f"📭 *No data found within the filter:* `{label}`\n_Try logging a transaction first!_")
        return
