...
# CRITICAL BUG FIX: Correcting namespace mismatch path
...
async def handle_report_command(bot: Bot, chat_id: int, command: str, uid: str):
    ...
    try:
        ...
        if not data:
            await bot.send_message(chat_id, f"ℹ️ No expenses found for *{label}*.", parse_mode="Markdown")
            return
        ...
        for item in data:
            ...
            alert = "⚠️" if amt > (total * 0.3) and total > 0 else " "
            msg += f"{desc:<13} | {amt:<6.0f} | {alert} {cat}\n"

        msg += f"{'-' * 32}\n```\n"
        msg += f"💰 *Total Spent: ₹{total:,.2f}*\n\n"
        msg += f"_Tip: ⚠️ indicates high-impact spending (>30% of period total)._"
...
async def handle_csv_export(bot: Bot, chat_id: int, uid: str, start_ts: float, end_ts: float):
    ...
    try:
        ...
        data = get_report_data(uid, start, end)
        if not data:
            await bot.send_message(chat_id, "❌ No data available for export.")
            return
        ...
        if 'mem_file' in locals(): mem_file.close()
        if 'byte_stream' in locals(): byte_stream.close()
...
async def handle_subscribe_command(bot, chat_id, text, uid):
    ...
    try:
        # CRITICAL BUG FIX: Reference the proper client layer directly
        from core.database import supabase
        supabase.table("report_schedules").upsert({
            "telegram_id": uid,
            "email": email,
            "is_active": True
