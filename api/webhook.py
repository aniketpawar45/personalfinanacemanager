...
def extract_remarks(q: dict) -> str:
    try:
        ...
        if raw_text.startswith(" Heard: "):
            return raw_text.replace(" Heard: ", "", 1).strip()
...
async def handle_webhook(request: Request, x_telegram_bot_api_secret_token: str = Header(None)):
    """
    CRITICAL SECURITY FIX: Intercept requests lacking our cryptographic token header
    to protect the application gateway against unauthorized request spoofing.
    """
    if TELEGRAM_WEBHOOK_SECRET and x_telegram_bot_api_secret_token != TELEGRAM_WEBHOOK_SECRET:
        logger.warning("Unauthorized webhook access attempt rejected.")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Inbound verification signature mismatch token check dropped."
        )
    ...
    try:
        ...
        if "callback_query" in update:
            ...
            if data.startswith("csv:"):
                ...
                last_cat_id = get_last_category(uid, desc)
                if last_cat_id:
        ...
        # --- INBOUND TEXT/VOICE MESSAGE MESSAGE ENGINE ---
            ...
            if "text" in msg:
                ...
                await bot.send_message(chat_id, "🤖 I only understand text parsing and voice messages.")
                return {"status": "ok"}
            ...
            if text:
                ...
                if text.startswith("/start"):
                    await bot.send_message(chat_id,
                                           "👋 Send expenses (e.g., 'Coffee 40, Bread 30') or record a Voice Note!")
                    ...
                    # Controlled access routing check logic parameter matching
                    await bot.send_message(chat_id, get_global_stats(uid), parse_mode="Markdown")
                elif text.startswith("/"):
                    await bot.send_message(chat_id, "❓ Unknown command. Use /report to see your ledger.")
                else:
                    # Run structured entity extraction engine
                    categories = get_all_categories()
                        ...
                        try:
                            if amt <= 0:
                                raise FinanceManagerException("AI Extraction Node",
                                                              f"No valid price parsed for '{desc}'.",
                                                              "Include a clear numerical amount.")
                            ...
                            error_msg = f"❌ **Error processing transaction '{desc}'**\n{fme.message}\n*Action:* {fme.action}"
                            await bot.send_message(chat_id=chat_id, text=error_msg, parse_mode="Markdown",
                                                   reply_to_message_id=active_msg_id)

    except FinanceManagerException as fme:
        error_msg = f"💥 **System Failure at [{fme.step}]**\n{fme.message}\n*Action:* {fme.action}"
        if chat_id: await bot.send_message(chat_id=chat_id, text=error_msg, parse_mode="Markdown")
    except Exception as e:
        error_msg = f"🚨 **Unexpected Critical Failure**\nInternal Workflow Processing Fault\n*Action:* Technical administrators notified."
        logger.error(f"Critical operational error at runtime context: {str(e)}", exc_info=True)
        if chat_id: await bot.send_message(chat_id=chat_id, text=error_msg, parse_mode="Markdown")
    finally:
