...
async def verify_user_authorization(bot_client: Bot, chat_id: int, telegram_id: str) -> bool:
    ...
    logger.warning(f"🔒 Security Firewall Triggered: Blocked access request from UID: {telegram_id}")
    denied_text = (
        "🔒 **Access Denied: Unregistered Connection**\n\n"
        "Your Telegram ID profile is not authorized to interact with this application.\n"
        "Please message the system administrator for deployment inclusion."
    )
    try:
        await bot_client.send_message(chat_id=chat_id, text=denied_text, parse_mode="Markdown")
    except Exception as e:
        logger.error(f"Failed to transmit security fallback statement: {str(e)}")
    return False
...
async def handle_webhook(request: Request):
    ...
    try:
        ...
        # --- 1. CALLBACK QUERY ROUTE INTERCEPTOR ---
        ...
        # --- 2. MESSAGE (TEXT/VOICE) ROUTE INTERCEPTOR ---
            ...
            if "text" in msg:
                ...
                heard_msg = await bot.send_message(chat_id, f"  Heard: {text}", reply_to_message_id=msg_id)
        ...
        error_msg = f"⚠️ **System Failure at [{fme.step}]**\n{fme.message}\n**Action:** {fme.action}"
        if chat_id:
            await bot.send_message(chat_id=chat_id, text=error_msg, parse_mode="Markdown")
    except Exception as e:
