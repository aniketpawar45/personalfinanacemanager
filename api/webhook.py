    ...
    try:
        if "callback_query" in update:
            ...
            if data.startswith("yes_future:"):
                ...
                if last_cat_id:
                    ...
                    if save_transaction(record):
                        ...
                        await bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                                    text="⚠️ **I'm sorry, I couldn't save that to the database right now.** Please try again in a moment.",
                                                    parse_mode="Markdown")
                else:
            ...
            if text.startswith("/start"):
                ...
                if amt <= 0:
                    await bot.send_message(chat_id,
                                           "⚠️ I couldn't quite understand the amount. Could you try formatting it a bit differently? (e.g., 'Coffee 40')")
                    ...
                    last_cat_id = get_last_category(desc)
                    if last_cat_id:
                        record = TransactionRecord(user_id=uid, amount=amt, category_id=last_cat_id, description=desc,
                                                   transaction_date=date)
                        # ✅ Check if save was actually successful
                        if save_transaction(record):
                            cats = get_all_categories()
                            ...
                            await bot.send_message(chat_id,
                                                   "⚠️ **I'm sorry, I encountered an issue saving your transaction.** Please try again later.",
                                                   parse_mode="Markdown")
                    else:
                        categories = get_all_categories()
                        buttons = [[InlineKeyboardButton(c['category_name'],
