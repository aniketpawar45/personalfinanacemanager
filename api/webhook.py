...
from api.reports import handle_report_command, handle_csv_export, handle_subscribe_command
        ...
        if "callback_query" in update:
            ...
            if data.startswith("csv:"):
                ...
                await bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                            text="Select a category for this unknown item:",
                ...
                if last_cat_id:
                    record = TransactionRecord(user_id=uid, amount=float(amt), category_id=last_cat_id,
                                               description=desc, transaction_date=date, remarks=remarks)
                    ...
                    buttons = [[InlineKeyboardButton(c['category_name'],
                               ...
                               for c in categories]
                    await bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                                text=f"Select a category for {desc}:",
                                                reply_markup=InlineKeyboardMarkup(buttons))
            ...
            if "text" in msg:
                ...
                async with httpx.AsyncClient() as http_client:
                    ...
                    if not file_info.get("ok"):
                        raise FinanceManagerException("Voice Fetching Node", "Could not access Telegram voice file.",
                                                      "Retry sending the voice note.")
            ...
            if text:
                ...
                # =====================================================================
                # ENTERPRISE FIREWALL: Strict Command Routing (Smart Command Interceptor)
                # =====================================================================
                ...
                elif text_lower.startswith("/stats") or text_lower.startswith("stats"):
                    await bot.send_message(chat_id, get_user_stats(uid), parse_mode="Markdown")

                elif text_lower.startswith("/allstats") or text_lower.startswith("allstats"):
                    if not is_admin:
                ...
                elif text_lower.startswith("/subscribe") or text_lower.startswith("subscribe"):
                    # Check if it's the newer simple subscription syntax or old syntax
                    # Standardizing syntax lookup format safely
                    formatted_text = text if text.startswith("/") else f"/{text}"
                    parts = text.split()

                    # If it uses email registration pattern: /subscribe your_email@domain.com
                    if len(parts) >= 2 and "@" in parts[1]:
                        await handle_subscribe_command(bot, chat_id, formatted_text, uid)
                    else:
                        # Legacy multi-parameter schedule handling route
                        try:
                            if len(parts) < 3:
                ...
                elif text_lower.startswith("/chart") or text_lower.startswith("chart"):
                    # Prepend forward slash for consistency if user typing text variant 'chart jan'
                    formatted_text = text if text.startswith("/") else f"/{text}"
                    await handle_chart_command(bot, chat_id, formatted_text, uid)

                elif text_lower.startswith("/statistics") or text_lower.startswith("statistics"):
                    formatted_text = text if text.startswith("/") else f"/{text}"
                    await handle_statistics_command(bot, chat_id, formatted_text, uid)

                elif text_lower.startswith("/report") or text_lower.startswith("report"):
                    # Critical isolation layer preventing plain text strings like 'report jan' from breaking things
                    formatted_text = text if text.startswith("/") else f"/{text}"
                    await handle_report_command(bot, chat_id, formatted_text, uid)

                elif text.startswith("/"):
                    await bot.send_message(chat_id, "⚠️ Unknown command. Use /report to see your ledger.")

                # =====================================================================
                # AI EXTRACTION PIPELINE: Safely isolated from user commands
                # =====================================================================
                else:
                    categories = get_all_categories()
                    ...
                    for amt, desc, date, ai_category in extracted_items:
                        try:
                            if amt <= 0:
                                raise FinanceManagerException("AI Extraction Node",
                                                              f"No valid price found for '{desc}'.",
                                                              "Include an amount.")

                            if desc == "Unknown Item":
                                kb = InlineKeyboardMarkup([
                                    [InlineKeyboardButton("Yes, save it",
                                                          callback_data=f"confirm_unk:{amt}:{date.timestamp()}")],
                                ...
                                await bot.send_message(chat_id,
                                                       ...
                                                       reply_markup=kb, reply_to_message_id=active_msg_id)

                            elif check_duplicate(uid, amt, desc):
                                await bot.send_message(chat_id, f"⚠️ Duplicate prevented for: {desc} - ₹{amt}",
                                                       reply_to_message_id=active_msg_id)

                            elif date > get_ist_now():
                                kb = InlineKeyboardMarkup([
                                    [InlineKeyboardButton("Yes",
                                                          callback_data=f"yes_future:{amt}:{desc[:10]}:{date.isoformat()}"),
                                     InlineKeyboardButton("No", callback_data="no_future")]
                                ])
                                await bot.send_message(chat_id,
                                                       f"Future date detected for '{desc}': {date.strftime('%d-%m-%Y')}. Are you sure?",
                                                       reply_markup=kb, reply_to_message_id=active_msg_id)
                                ...
                                if last_cat_id:
                                    ...
                                    cat_name_to_display = next(
                                        (c['category_name'] for c in categories if c['id'] == last_cat_id), "Other")
                                elif ai_category:
                                    for c in categories:
                                ...
                                if cat_id_to_use:
                                    record = TransactionRecord(user_id=uid, amount=amt, category_id=cat_id_to_use,
                                                               description=desc, transaction_date=date, remarks=text)
                                    save_transaction(record)
                                    await bot.send_message(chat_id,
                                                           f"✅ Auto-Saved: {desc} - ₹{amt} ({cat_name_to_display})",
                                                           reply_to_message_id=active_msg_id)
                                else:
                                    buttons = [[InlineKeyboardButton(c['category_name'],
                                                                     callback_data=f"cat:{c['id']}:{amt}:{desc[:10]}:{date.timestamp()}")]
                                               for c in categories]
                                    await bot.send_message(chat_id,
                                                           f"🤖 Unsure of category.\nSelect a category for '{desc}' (₹{amt}):",
                                                           reply_markup=InlineKeyboardMarkup(buttons),
                                                           reply_to_message_id=active_msg_id)
