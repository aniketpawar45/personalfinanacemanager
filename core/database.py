...
def check_duplicate(user_id, amount, description):
    ...
    res = supabase.table("transactions").select("id").eq("user_id", str(user_id)).eq("amount", amount).eq("description", description).gt("created_at", ten_seconds_ago).execute()
    return len(res.data) > 0

def save_transaction(user_id, amount, category_id, description, date):
    data = {"user_id": str(user_id), "amount": amount, "category_id": category_id, "description": description, "created_at": date.isoformat()}
